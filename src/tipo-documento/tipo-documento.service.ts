...
async findOne(idTipoDocumento: string): Promise<TipoDocumento> {
  const TipoDocumento = await this.tipoDocumentoRepository.findOne({ where: { idTipoDocumento } });
  if (!TipoDocumento) {
      throw new NotFoundException(`Persona with ID ${idTipoDocumento} not found`);
  }
...
async update(idTipoDocumento: string, updateTipoDocumentoDto: UpdateTipoDocumentoDto): Promise<TipoDocumento> {
    try {
...
async remove(idTipoDocumento: string): Promise<void> {
    const tipoDocumento = await this.findOne(idTipoDocumento);
